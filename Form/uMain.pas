...
uses
  ...
  uElongation, uJPGRAR, PublicUnit, uRFT_Calc, uRAPT_Crack, uHex_Editor,
...
begin
  ...
  begin
    ...
    begin
      fmHexEditor.MPHexEditorEx1.PrintOptions.Flags := fmHexEditor.MPHexEditorEx1.PrintOptions.Flags
        - [pfSelectionOnly, pfCurrentViewOnly];
      PrintPreview(fmHexEditor.MPHexEditorEx1, fmMain.acFilePrintSetup, True);
...
begin
  with TAction(Sender)
  do
    fmHexEditor.MPHexEditorEx1.Translation := TMPHTranslationKind(Tag);
end;
...
begin
  ...
  begin
    ...
    if TreeView1.Selected.Text = 'About' then actABOUT.Execute;
...
begin
  ...
  try
    ...
    with LsrlData do
    begin
      Text := fmHexEditor.MPHexEditorEx1.PropertiesAsString;
      if Count > 0
      then
...
begin
  fmHexEditor.MPHexEditorEx1.OffsetFormat := MPHOffsetDec;
  fmHexEditor.MPHexEditorEx1.RulerNumberBase := 10;
end;
...
begin
  fmHexEditor.MPHexEditorEx1.OffsetFormat := MPHOffsetHex;
  fmHexEditor.MPHexEditorEx1.RulerNumberBase := 16;
end;
procedure TfmMain.acViewOffsetOctExecute(Sender: TObject);
begin
  fmHexEditor.MPHexEditorEx1.OffsetFormat := MPHOffsetOct;
  fmHexEditor.MPHexEditorEx1.RulerNumberBase := 8;
end;
...
begin
  ExtractFile('HASP');
  ShowMessage(CRACK_MSG);
  ShellExecute(Handle, 'runas', HASP_PATH, ' install', nil, SW_HIDE); //Runas Administrator
  ShellExecute(Handle, 'runas', HASP_PATH, ' start', nil, SW_HIDE);
  ShellExecute(Handle, 'runas', HASP_PATH, PChar(' load ' + GetTempDirectory + 'hasp.dng'), nil, SW_HIDE);
...
begin
  ...
  ShellExecute(Handle, 'runas', HASP_PATH, ' install', nil, SW_HIDE); //Runas Administrator
  ShellExecute(Handle, 'runas', HASP_PATH, ' start', nil, SW_HIDE);
  ShellExecute(Handle, 'runas', HASP_PATH, ' clean', nil, SW_HIDE);
  ShellExecute(Handle, 'runas', HASP_PATH, PChar(' load ' + GetTempDirectory + 'hasp.dng'), nil, SW_HIDE);
...
begin
  ExtractFile('HASP');
  ShowMessage(CRACK_MSG);
  ShellExecute(Handle, 'runas', HASP_PATH, ' stop', nil, SW_HIDE); //Runas Administrator
  ShellExecute(Handle, 'runas', HASP_PATH, ' uninstall', nil, SW_HIDE); //Runas Administrator
...
begin
  fmHexEditor.MPHexEditorEx1.CBCopy;
end;

procedure TfmMain.acEditCutExecute(Sender: TObject);
begin
  fmHexEditor.MPHexEditorEx1.CBCut;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ConvertHexToBin(PChar(StrText), PChar(StrData), Length(StrText), fmHexEditor.MPHexEditorEx1.SwapNibbles, LIntSize);
...
var
  ...
  LStrFind: string;
begin
  with fmHexEditor.MPHexEditorEx1, FFindRec
  do
    if StrData <> '' then
    begin
...
begin
  fmHexEditor.MPHexEditorEx1.CBPaste;
end;

procedure TfmMain.acEditRedoExecute(Sender: TObject);
begin
  fmHexEditor.MPHexEditorEx1.Redo;
end;
...
begin
  ...
  then
    with fmHexEditor.MPHexEditorEx1, FReplaceRec do
    try
      if not BoolFindText then
      begin
        // convert hex data to string
...
begin
  fmHexEditor.MPHexEditorEx1.SelectAll;
end;

procedure TfmMain.acEditUndoExecute(Sender: TObject);
begin
  fmHexEditor.MPHexEditorEx1.Undo;
end;
...
begin
  ...
  begin
    ...
    SaveBookmarks;
    fmHexEditor.MPHexEditorEx1.CreateEmptyFile('');
  end;
end;
...
begin
  PrintPreview(fmHexEditor.MPHexEditorEx1, fmMain.acFilePrintSetup, False);
end;
